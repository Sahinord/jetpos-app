...
/**
 * Hepsiburada Order/Package API (server-only).
 * Tek bir "Order API" hem pazaryeri siparişlerini hem de kargo firması
 * seçimini (HepsiJet dahil) ve etiket alımını kapsıyor — Trendyol'un aksine
 * ayrı bir "kargo API"si yok, bu yüzden ayrı bir HepsiJet client'ı gerekmiyor.
 * Endpoint/auth şekli açık kaynak referans implementasyonundan doğrulandı
 * (resmi developer portalı bot korumasından dolayı doğrudan okunamadı).
 */
...
// Katalog Ürün Girişi (kategori/özellik/ürün durumu) — mpop(-sit).hepsiburada.com
...
/**
 * Ürün Bilgisi Gönderme — POST /product/api/products/import, multipart/form-data,
 * dosya alan adı "file" (.json uzantılı). Resmi sayfa bot korumasından doğrudan
 * okunamadığı için arama motoru özeti üzerinden doğrulandı — canlıya geçmeden
 * önce SIT ortamında bir kez denenip teyit edilmesi önerilir.
 */
