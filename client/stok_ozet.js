// Stok gerçeğini DB'den ADET/KG/PASİF ayrı gösterir. client/ içinde çalıştır:
//   cd client && node stok_ozet.js            → tüm işletmeler
//   cd client && node stok_ozet.js <tenant_id>
...
function summarize(products) {
    ...
    const s = { adetAktif: 0, adetAktifStok: 0, kgAktif: 0, kgAktifStok: 0, pasif: 0, pasifStok: 0, toplamHam: 0 };
...
function print(name, s) {
    ...
    console.log(`\n=== ${name} ===`);
    console.log(`  ADET (aktif) : ${s.adetAktif} urun -> toplam ${r(s.adetAktifStok)} adet   <-- PC "TOPLAM STOK" bunu gosterir`);
    console.log(`  KG   (aktif) : ${s.kgAktif} urun -> toplam ${r(s.kgAktifStok)} kg`);
    console.log(`  PASIF        : ${s.pasif} urun -> toplam ${r(s.pasifStok)}`);
    console.log(`  ------------`);
    console.log(`  Eski mobil (adet+kg+pasif KARISIK) : ${r(s.toplamHam)}   <-- "3294,55" buydu`);
    console.log(`  Dogru adet toplami                 : ${r(s.adetAktifStok)}`);
}
