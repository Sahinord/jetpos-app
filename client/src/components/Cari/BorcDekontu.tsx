...
import { Save, X, Trash2, Copy, Printer, FileText, Plus, Search, Building } from "lucide-react";
import { supabase } from "@/lib/supabase";
import { useTenant } from "@/lib/tenant-context";

interface BorcDekontuProps {
...
export default function BorcDekontu({ showToast }: BorcDekontuProps) {
    ...
    useEffect(() => {
        ...
        const now = new Date();
        const fisNo = `BD-${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}-${String(now.getHours()).padStart(2, '0')}${String(now.getMinutes()).padStart(2, '0')}`;
    ...
    const handlePrint = () => {
        ...
        const tableRows = kalemler.map((kalem, i) => `
            <tr>
                ...
                <td style="text-align: right;">${(kalem.tutar || 0).toLocaleString('tr-TR', { minimumFractionDigits: 2 })} ${kalem.paraBirimi}</td>
            </tr>
        `).join('');
            ...
            <html>
                ...
                <body>
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            ${tableRows || '<tr><td colspan="4" style="text-align: center;">Kalem bulunamadı</td></tr>'}
                            <tr class="total-row">
                                <td colspan="3" style="text-align: right;">TOPLAM:</td>
                                <td style="text-align: right;">${toplamTutar.toLocaleString('tr-TR', { minimumFractionDigits: 2 })} ${formData.paraBirimi}</td>
                            </tr>
                        </tbody>
                    </table>
                                    ...
                                    {kalemler.length === 0 && (
                                        <tr>
                                            <td colSpan={9} className="text-center py-8 text-secondary">
                                                Henüz kalem eklenmedi. <button onClick={addKalem} className="text-primary hover:underline">Kalem ekle</button>
                                            </td>
                                        </tr>
                                    )}
                                </tbody>
