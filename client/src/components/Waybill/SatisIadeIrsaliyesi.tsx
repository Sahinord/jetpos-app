...
export default function SatisIadeIrsaliyesi() {
    ...
    const saveWaybill = async () => {
        ...
        try {
            ...
            // Cari hesaba borç yaz (iade olduğu için ters işlem)
            await supabase.from('cari_hareketler').insert({
                tenant_id: currentTenant?.id,
                cari_id: waybill.cari_id,
                hareket_tipi: 'borclandirma',
                aciklama: `Satış İade İrsaliyesi: ${nextNumber}${waybill.return_reason ? ' - ' + waybill.return_reason : ''}`,
                tutar: waybill.grand_total,
                islem_tarihi: waybill.waybill_date,
                belge_no: nextNumber,
                belge_tipi: 'Satış İade İrsaliyesi'
            });

            alert('✅ Satış iade irsaliyesi başarıyla kaydedildi!');
        ...
        <div className="space-y-4 max-w-[1800px] mx-auto p-4">
            ...
            <div className="flex items-center justify-end">
                <button
                    onClick={saveWaybill}
                    disabled={loading}
                    className="interactive-button bg-orange-500 hover:bg-orange-600 text-white px-6 py-2 rounded-xl text-sm font-bold flex items-center gap-2 shadow-lg transition-all active:scale-95 disabled:opacity-50"
                >
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div className="md:col-span-2 space-y-2">
                        ...
                        <div className="relative">
                            <button
                                ...
                                className="w-full bg-background border-2 border-orange-500/30 rounded-lg px-3 py-2.5 text-left text-sm font-medium text-foreground hover:border-orange-500 transition-all flex items-center justify-between"
                            >
                    ...
                    <div className="space-y-2">
                        ...
                        <input
                            ...
                            className="w-full bg-background border border-border rounded-lg px-3 py-2.5 text-sm text-foreground outline-none focus:border-orange-500"
                        />
                    </div>
                    ...
                    <div className="space-y-2">
                        <label className="text-xs font-bold text-orange-500 uppercase">Orijinal İrsaliye No</label>
                        <input
                            type="text"
                            value={waybill.original_waybill_no}
                            onChange={(e) => setWaybill(prev => ({ ...prev, original_waybill_no: e.target.value }))}
                            placeholder="SI2026000001"
                            className="w-full bg-background border border-orange-500/30 rounded-lg px-3 py-2.5 text-sm text-foreground outline-none focus:border-orange-500"
                        />
                    </div>

                    <div className="md:col-span-3 space-y-2">
                        <label className="text-xs font-bold text-orange-500 uppercase">İade Nedeni *</label>
                        <input
                            type="text"
                            value={waybill.return_reason}
                            onChange={(e) => setWaybill(prev => ({ ...prev, return_reason: e.target.value }))}
                            placeholder="Örn: Hasarlı ürün, Yanlış gönderim, Müşteri talebi..."
                            className="w-full bg-background border border-orange-500/30 rounded-lg px-3 py-2.5 text-sm text-foreground outline-none focus:border-orange-500"
                        />
                    </div>
                </div>
            ...
            <div className="glass-card animate-in p-4 space-y-4" style={{ animationDelay: '0.1s' }}>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-xs border-collapse">
                        <thead className="bg-orange-500/10 text-secondary font-bold uppercase border-b-2 border-orange-500/30">
                            <tr>
                                ...
                                <th className="px-3 py-3 text-right w-36 bg-orange-500/10">Toplam</th>
                                ...
                                <tr key={index} className="hover:bg-white/[0.02]">
                                    ...
                                    <td className="p-2">
                                        <input
                                            ...
                                            className="w-full bg-orange-500/10 border border-orange-500/30 rounded-lg px-2 py-2 text-orange-500 font-bold text-center outline-none focus:border-orange-500"
                                        />
                                    </td>
                                    <td className="p-2">
                                        <select
                                    ...
                                    <td className="p-2 text-right font-black text-foreground bg-orange-500/5 font-mono">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="glass-card animate-in p-4 space-y-3 bg-gradient-to-br from-orange-500/5 to-transparent" style={{ animationDelay: '0.3s' }}>
                    ...
                    <div className="space-y-3">
                        <div className="grid grid-cols-2 gap-3">
                            ...
                            <div className="bg-background/50 rounded-lg p-3 border border-orange-500/30">
                                <div className="text-[10px] text-orange-500 uppercase font-bold mb-1">KDV Toplamı</div>
                                <div className="text-lg font-black text-orange-500 font-mono">+{formatCurrency(waybill.total_vat || 0)}</div>
                            </div>
                        </div>

                        <div className="bg-orange-500/20 rounded-xl border-2 border-orange-500/40 p-4">
                            <div className="text-xs font-black text-orange-500 uppercase tracking-wider mb-2">İADE TOPLAMI</div>
