...
import {
    Package, Plus, Save, Search, Trash2, Calculator, RotateCcw
} from 'lucide-react';
...
export default function AlisIadeIrsaliyesi() {
    ...
    const saveWaybill = async () => {
        ...
        try {
            const { data: nextNumber } = await supabase.rpc('get_next_waybill_number', {
                ...
                p_waybill_type: 'purchase_return'
            });
            ...
            // Cari hesaba alacak yaz (iade olduğu için ters işlem)
            await supabase.from('cari_hareketler').insert({
                tenant_id: currentTenant?.id,
                cari_id: waybill.cari_id,
                hareket_tipi: 'alacaklandirma',
                aciklama: `Alış İade İrsaliyesi: ${nextNumber}${waybill.return_reason ? ' - ' + waybill.return_reason : ''}`,
                tutar: waybill.grand_total,
                islem_tarihi: waybill.waybill_date,
                belge_no: nextNumber,
                belge_tipi: 'Alış İade İrsaliyesi'
            });

            alert('✅ Alış iade irsaliyesi başarıyla kaydedildi!');
        ...
        <div className="space-y-4 max-w-[1800px] mx-auto p-4">
            ...
            <div className="flex items-center justify-end">
                <button
                    onClick={saveWaybill}
                    disabled={loading}
                    className="bg-purple-500 hover:bg-purple-600 text-white px-6 py-2 rounded-xl text-sm font-bold flex items-center gap-2 shadow-lg transition-all active:scale-95 disabled:opacity-50"
                >
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div className="md:col-span-2 space-y-2">
                        ...
                        <div className="relative">
                            <button
                                ...
                                className="w-full bg-background border-2 border-purple-500/30 rounded-lg px-3 py-2.5 text-left text-sm font-medium text-foreground hover:border-purple-500 transition-all flex items-center justify-between"
                            >
                                ...
                                <div className="absolute z-50 top-full mt-2 w-full bg-card border border-border rounded-xl shadow-2xl max-h-80 overflow-y-auto">
                                    <div className="sticky top-0 bg-card p-3 border-b border-border">
                                        <input
                                            ...
                                            className="w-full bg-background border border-border rounded-lg px-3 py-2 text-sm outline-none focus:border-purple-500"
                                            autoFocus
                                        />
                                    </div>
                                    <div>
                                        {filteredCariList.map(cari => (
                                            <button
                                                ...
                                                className="w-full px-4 py-3 text-left hover:bg-purple-500/10 transition-colors border-b border-border/50 last:border-0"
                                            >
                                    ...
                                    <div className="sticky bottom-0 bg-card p-2 border-t border-border">
                                        <button
                                            ...
                                            className="w-full px-3 py-2 bg-purple-500/10 hover:bg-purple-500/20 text-purple-500 rounded-lg text-xs font-bold transition-all"
                                        >
                                            Kapat
                                        </button>
                                    </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <input
                            ...
                            className="w-full bg-background border border-border rounded-lg px-3 py-2.5 text-sm text-foreground outline-none focus:border-purple-500"
                        />
                    </div>
                    ...
                    <div className="space-y-2">
                        <label className="text-xs font-bold text-purple-500 uppercase">Orijinal İrsaliye No</label>
                        <input
                            type="text"
                            value={waybill.original_waybill_no}
                            onChange={(e) => setWaybill(prev => ({ ...prev, original_waybill_no: e.target.value }))}
                            placeholder="AI2026000001"
                            className="w-full bg-background border border-purple-500/30 rounded-lg px-3 py-2.5 text-sm text-foreground outline-none focus:border-purple-500"
                        />
                    </div>

                    <div className="md:col-span-3 space-y-2">
                        <label className="text-xs font-bold text-purple-500 uppercase">İade Nedeni *</label>
                        <input
                            type="text"
                            value={waybill.return_reason}
                            onChange={(e) => setWaybill(prev => ({ ...prev, return_reason: e.target.value }))}
                            placeholder="Örn: Hasarlı ürün, Eksik gönderim, Kalite problemi..."
                            className="w-full bg-background border border-purple-500/30 rounded-lg px-3 py-2.5 text-sm text-foreground outline-none focus:border-purple-500"
                        />
                    </div>
                </div>
            ...
            <div className="glass-card p-4 space-y-4">
                <div className="flex items-center justify-between border-b border-border pb-3">
                    <h2 className="text-sm font-bold text-purple-500 uppercase tracking-wider flex items-center gap-2">
                        <Package className="w-4 h-4" />
                        İade Edilen Kalemler
                    </h2>
                    <button
                        onClick={addItem}
                        className="text-purple-500 hover:text-purple-400 text-xs font-bold flex items-center gap-1 transition-all"
                    >
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-xs border-collapse">
                        <thead className="bg-purple-500/10 text-secondary font-bold uppercase border-b-2 border-purple-500/30">
                            <tr>
                                ...
                                <th className="px-3 py-3 text-right w-36 bg-purple-500/10">Toplam</th>
                                ...
                                <tr key={index} className="hover:bg-white/[0.02]">
                                    ...
                                    <td className="p-2">
                                        <input
                                            ...
                                            className="w-full bg-purple-500/10 border border-purple-500/30 rounded-lg px-2 py-2 text-purple-500 font-bold text-center outline-none focus:border-purple-500"
                                        />
                                    </td>
                                    <td className="p-2">
                                        <select
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="glass-card p-4 space-y-3 bg-gradient-to-br from-purple-500/5 to-transparent">
