        ...
        try {
            ...
            const { data: waybillData, error } = await supabase
                ...
                .insert({
                    ...
                    notes: `${waybill.order_no ? 'Sipariş No: ' + waybill.order_no + '\n' : ''}${waybill.tracking_no ? 'Takip No: ' + waybill.tracking_no + '\n' : ''}${waybill.shipping_company ? 'Kargo: ' + waybill.shipping_company + '\n' : ''}${waybill.notes}`,
                    status: 'approved',
        ...
        <div className="space-y-4 max-w-[1800px] mx-auto p-4">
            ...
            <div className="flex items-center justify-end">
                <button
                    onClick={saveWaybill}
                    disabled={loading}
                    className="bg-cyan-500 hover:bg-cyan-600 text-white px-6 py-2 rounded-xl text-sm font-bold flex items-center gap-2 shadow-lg transition-all active:scale-95 disabled:opacity-50"
                >
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div className="md:col-span-2 space-y-2">
                        ...
                        <div className="relative">
                            <button
                                ...
                                className="w-full bg-background border-2 border-cyan-500/30 rounded-lg px-3 py-2.5 text-left text-sm font-medium text-foreground hover:border-cyan-500 transition-all flex items-center justify-between"
                            >
                                ...
                                <div className="absolute z-50 top-full mt-2 w-full bg-card border border-border rounded-xl shadow-2xl max-h-80 overflow-y-auto">
                                    <div className="sticky top-0 bg-card p-3 border-b border-border">
                                        <input
                                            ...
                                            className="w-full bg-background border border-border rounded-lg px-3 py-2 text-sm outline-none focus:border-cyan-500"
                                            autoFocus
                                        />
                                    </div>
                                    <div>
                                        {filteredCariList.map(cari => (
                                            <button
                                                ...
                                                className="w-full px-4 py-3 text-left hover:bg-cyan-500/10 transition-colors border-b border-border/50 last:border-0"
                                            >
                                    ...
                                    <div className="sticky bottom-0 bg-card p-2 border-t border-border">
                                        <button
                                            ...
                                            className="w-full px-3 py-2 bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-500 rounded-lg text-xs font-bold transition-all"
                                        >
                                            Kapat
                                        </button>
                                    </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <input
                            ...
                            className="w-full bg-background border border-border rounded-lg px-3 py-2.5 text-sm text-foreground outline-none focus:border-cyan-500"
                        />
                    </div>
                    ...
                    <div className="space-y-2">
                        <label className="text-xs font-bold text-cyan-500 uppercase">Sipariş No</label>
                        <input
                            type="text"
                            value={waybill.order_no}
                            onChange={(e) => setWaybill(prev => ({ ...prev, order_no: e.target.value }))}
                            placeholder="SIP2026000001"
                            className="w-full bg-background border border-cyan-500/30 rounded-lg px-3 py-2.5 text-sm text-foreground outline-none focus:border-cyan-500"
                        />
                    </div>

                    <div className="space-y-2">
                        <label className="text-xs font-bold text-cyan-500 uppercase">Kargo Takip No</label>
                        <input
                            type="text"
                            value={waybill.tracking_no}
                            onChange={(e) => setWaybill(prev => ({ ...prev, tracking_no: e.target.value }))}
                    ...
                    <div className="space-y-2">
                        <label className="text-xs font-bold text-cyan-500 uppercase">Kargo Firması</label>
                        <input
                            type="text"
                            value={waybill.shipping_company}
                            onChange={(e) => setWaybill(prev => ({ ...prev, shipping_company: e.target.value }))}
                            placeholder="Aras, Yurtiçi, MNG vb."
                            className="w-full bg-background border border-cyan-500/30 rounded-lg px-3 py-2.5 text-sm text-foreground outline-none focus:border-cyan-500"
                        />
                    </div>

                    <div className="md:col-span-4 space-y-2">
                        <label className="text-xs font-bold text-secondary uppercase">Teslimat Adresi</label>
                        <textarea
                            value={waybill.delivery_address}
                            onChange={(e) => setWaybill(prev => ({ ...prev, delivery_address: e.target.value }))}
                            placeholder="Teslimat adresi detayı..."
                            className="w-full bg-background border border-border rounded-lg px-3 py-2 text-sm text-foreground outline-none focus:border-cyan-500 resize-none"
                            rows={2}
                        />
                    </div>
                </div>
            ...
            <div className="glass-card p-4 space-y-4">
                <div className="flex items-center justify-between border-b border-border pb-3">
                    ...
                    <button
                        onClick={addItem}
                        className="text-cyan-500 hover:text-cyan-400 text-xs font-bold flex items-center gap-1 transition-all"
                    >
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-xs border-collapse">
                        <thead className="bg-cyan-500/10 text-secondary font-bold uppercase border-b-2 border-cyan-500/30">
                            <tr>
                                ...
                                <th className="px-3 py-3 text-right w-36 bg-cyan-500/10">Toplam</th>
                                ...
                                <tr key={index} className="hover:bg-white/[0.02]">
                                    ...
                                    <td className="p-2">
                                        <input
                                            ...
                                            className="w-full bg-cyan-500/10 border border-cyan-500/30 rounded-lg px-2 py-2 text-cyan-500 font-bold text-center outline-none focus:border-cyan-500"
                                        />
                                    </td>
                                    <td className="p-2">
                                        <select
                                    ...
                                    <td className="p-2 text-right font-black text-foreground bg-cyan-500/5 font-mono">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="glass-card p-4 space-y-3 bg-gradient-to-br from-cyan-500/5 to-transparent">
                    ...
                    <div className="space-y-3">
                        <div className="grid grid-cols-2 gap-3">
                            ...
                            <div className="bg-background/50 rounded-lg p-3 border border-cyan-500/30">
                                <div className="text-[10px] text-cyan-500 uppercase font-bold mb-1">KDV Toplamı</div>
                                <div className="text-lg font-black text-cyan-500 font-mono">+{formatCurrency(waybill.total_vat || 0)}</div>
                            </div>
                        </div>

                        <div className="bg-cyan-500/20 rounded-xl border-2 border-cyan-500/40 p-4">
                            <div className="text-xs font-black text-cyan-500 uppercase tracking-wider mb-2">SEVKİYAT TOPLAMI</div>
                            <div className="text-3xl font-black text-cyan-500 font-mono tracking-tight">
