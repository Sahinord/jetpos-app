...
export async function POST(req: NextRequest) {
    try {
        ...
        const { beginDate, endDate, page = 0, size = 50 } = body;

        const path = `/orders/merchantid/${creds.merchantId}${qs({ beginDate, endDate, page, size })}`;
        const data = await hbOrderRequest(creds, path, { method: 'GET' });

        return NextResponse.json(data);
    } catch (error: any) {
        console.error('Hepsiburada Orders Exception:', error);
